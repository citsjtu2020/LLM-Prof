...
class OEAEfficiencyAnalyzerSGLang:
    def __init__(self):
        """初始化SGLang框架的OEA效率分析器"""
        
        # SGLang的Linear算子类型（从Stage 2的projection_summary获取）
    ...
    def extract_linear_data_from_stage2(self, proj_name: str, stage2_data: Dict) -> Dict:
        """从Stage 2的projection_summary中提取Linear算子数据"""
        ...
        # 从linear_analysis.projection_summary获取
        linear_analysis = stage2_data.get('linear_analysis', {})
        projection_summary = linear_analysis.get('projection_summary', {})
        
        if proj_name in projection_summary:
            proj_data = projection_summary[proj_name]
            operator_data['total_flops'] = proj_data.get('total_flops', 0)
            operator_data['total_memory_access'] = proj_data.get('total_memory_access', 0) / 1e9  # 转GB
            operator_data['total_duration_us'] = proj_data.get('total_duration_us', 0)
            operator_data['kernel_count'] = proj_data.get('execution_count', 0)
            operator_data['data_source'] = 'stage2_projection_summary'
    ...
    def extract_operator_data_from_stage1(self, op_type: str, stage1_data: Dict) -> Dict:
        ...
        operator_kernels = stage1_data.get('operator_kernels', {})
        
        if op_type in operator_kernels:
            kernels = operator_kernels[op_type]
    ...
    def analyze_comprehensive_efficiency(self, stage1_data: Dict, stage2_data: Optional[Dict],
        ...
        for proj_name in self.linear_projections:
            ...
            if operator_data['kernel_count'] > 0:
                print(f"{proj_name:12s}: {operator_data['kernel_count']:4d} kernels, "
        ...
        for op_type in self.nonlinear_types:
            ...
            if operator_data['kernel_count'] > 0:
                print(f"{op_type:12s}: {operator_data['kernel_count']:4d} kernels, "
                      f"{operator_data['total_duration_us']/1000:6.1f} ms, "
                      f"数据源: {operator_data['data_source']}")
        ...
        # 显示结果
        print(f"{'算子类型':12s} {'类别':10s} {'瓶颈评分':>8s} {'效率度':>8s} "
        ...
        for item in bottleneck_ranking:
            ...
            print(f"{item['operator_type']:12s} {category_short:10s} "
        ...
        return {
            ...
            'analysis_version': 'oea_sglang_stage4'
        }
    ...
    def generate_report(self, analysis_results: Dict) -> str:
        ...
        report = []
        report.append("=" * 80)
        report.append("OEA Stage 4: SGLang算子效率分析报告")
        report.append("=" * 80)
        ...
        # 瓶颈分析
        report.append(f"\n瓶颈分析 (Top 10):")
        report.append(f"{'算子':12s} {'类别':10s} {'瓶颈评分':>8s} {'效率度':>8s} "
                     f"{'时间占比':>8s} {'类型':>8s}")
        report.append("-" * 70)
        ...
        for item in analysis_results['bottleneck_ranking'][:10]:
            ...
            report.append(f"{item['operator_type']:12s} {category_short:10s} "
                         f"{item['bottleneck_score']:8.3f} {item['efficiency_degree']:8.3f} "
...
def main():
    parser = argparse.ArgumentParser(description='OEA Stage 4: SGLang算子效率分析器')
    ...
    try:
        # 创建分析器
        analyzer = OEAEfficiencyAnalyzerSGLang()
