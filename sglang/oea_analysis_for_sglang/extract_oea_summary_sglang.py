...
使用方法:
python extract_oea_summary_sglang.py \
...
class OEASummaryExtractorSGLang:
    def __init__(self):
        """初始化SGLang OEA结果提取器"""
        print("=== SGLang OEA结果数据提取器 ===")
        
        # Linear算子列表
        self.linear_projections = ['qkv_proj', 'o_proj', 'gate_proj', 'up_proj', 'down_proj', 'lm_head']
    ...
    def _build_end_to_end_info(self, full_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'inference_start_time': 0,  # SGLang Stage4没有这个信息
            'inference_end_time': overall.get('total_kernel_time_us', 0),
            'data_source': 'stage4_sglang'
        }
    ...
    def _build_time_breakdown(self, full_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        total_end_to_end = total_kernel_time  # SGLang没有idle time信息
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='SGLang OEA结果数据提取器')
    ...
    try:
        ...
        extractor = OEASummaryExtractorSGLang()
