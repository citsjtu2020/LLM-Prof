#!/usr/bin/env python3
"""
vLLM框架 LLM-Prof 数据整合脚本
整合vLLM框架案例的SEA、MEA、OEA三层数据，用于横向对比分析
数据源：
1. SEA+MEA: oea_analysis_for_vllm/cases_after_sea_with_mea.csv
...
class VLLMDataIntegrator:
    ...
    def load_sea_mea_data(self) -> pd.DataFrame:
        """加载SEA+MEA数据"""
        csv_file = self.base_dir / "oea_analysis_for_vllm" / "cases_after_sea_with_mea.csv"
    ...
    def parse_case_name(self, case_name: str) -> Dict[str, Any]:
        ...
        例如: Qwen2.5-3B_batch8_input128_output10
        """
    ...
    def extract_key_metrics(self, case_data: Dict) -> Dict:
        ...
        metrics['gpu_num'] = 1  # vLLM案例默认为1
        
        # 分组信息 - vLLM案例暂时不分组
        metrics['group_name'] = 'vLLM Framework'
        metrics['group_id'] = 0
        ...
        if oea_data:
            ...
            peak_bandwidth = hw_specs.get('mem_bandwidth_gbps', 1) * (1024**3)  # 转换为B/s
            
            if total_time_s > 0 and peak_bandwidth > 0:
                actual_bandwidth = total_memory_access / total_time_s
                metrics['oea_memory_utilization'] = actual_bandwidth / peak_bandwidth
            else:
    ...
    def export_to_csv(self, integrated_data: List[Dict], output_file: str = "vllm_integrated_data.csv"):
        """导出为CSV格式，保持与rtp-llm框架一致的列顺序"""
        print(f"\n📤 导出数据到 {output_file}...")
    ...
    def export_to_json(self, integrated_data: List[Dict], output_file: str = "vllm_integrated_data.json"):
        """导出为JSON格式"""
        print(f"\n📤 导出完整数据到 {output_file}...")
...
def main():
    """主函数"""
    print("🎯 vLLM框架 LLM-Prof 数据整合工具")
    print("=" * 60)
    
    # 创建数据整合器
    integrator = VLLMDataIntegrator()
