...
class SGLangOEADataProcessor:
    ...
    def process_case(self, csv_file: str, iteration_json: str, trace_file: str = None) -> Dict:
        ...
        # 如果提供了trace_file参数，直接使用传入的文件路径
        if trace_file and os.path.exists(trace_file) and os.path.exists(iteration_json):
            print(f"使用提供的文件路径:")
            print(f"  - trace_file: {trace_file}")
            print(f"  - iteration_json: {iteration_json}")
            ...
            # 直接处理文件，不再重新查找
            return self._process_files_directly(csv_file, inferred_pod_name, trace_file, iteration_json)
        
        # 否则使用原来的逻辑（从文件名推断并重新查找）
    ...
    def _process_files_directly(self, csv_file: str, pod_name: str, trace_file: str, iteration_json: str) -> Dict:
        """直接使用提供的文件路径处理案例，不重新查找文件"""
        
        print(f"=== 直接处理文件 ===")
        print(f"Pod name: {pod_name}")
        print(f"Trace file: {trace_file}")
        print(f"Iteration JSON: {iteration_json}")
        ...
        # 查找匹配的案例
        case_row = df[df['pod_name'] == pod_name]
        
        if case_row.empty:
            raise ValueError(f"在 CSV 中找不到 pod_name '{pod_name}' 的案例")
        
        case_info = self.extract_case_info_from_csv_row(case_row.iloc[0])
        print(f"找到案例: {case_info}")
        ...
        # 按 iteration 提取 kernels
        iteration_kernels = self.extract_kernels_by_iteration(trace_events, iteration_data)
        
        # 将 kernels 分配到每个 iteration 中
        print(f"\n=== 将 kernels 分配到每个 iteration ===")
        for iteration in iteration_data['iterations']:
            ...
            # 找到这个 iteration 时间范围内的 kernels
            iteration['kernels'] = []
            for kernel in trace_events:
                if (kernel.get('ph') == 'X' and  # Complete events
                    kernel.get('cat', '').lower() in ['kernel', 'gpu', 'cuda'] and
                    'name' in kernel and 'ts' in kernel and 'dur' in kernel):
                    kernel_ts = kernel['ts']
                    if start_ts <= kernel_ts <= end_ts:
                        # 添加算子分类信息
                        kernel_with_type = kernel.copy()
                        kernel_with_type['operator_type'] = self.classify_operator(kernel['name'])
                        kernel_with_type['phase'] = phase
                        iteration['kernels'].append(kernel_with_type)
            
            print(f"Iteration {iteration_id} ({phase}): 分配了 {len(iteration['kernels'])} 个 kernels")
        
        print(f"所有 iterations 的 kernels 分配完成")
