#!/usr/bin/env python3
"""
OEA Stage 2: Linear算子专门分析脚本 (修复版本)
基于原有stage2脚本，专门负责Linear算子的精确分析
使用每个kernel的精确token size值而不是平均值
...
使用方法:
python stage2_linear_analyzer_fixed.py --input stage1_processed_data_fixed.json
"""
...
class LinearAnalyzerFixed:
    ...
    def find_transformer_layers(self, all_kernels: List[Dict]) -> Tuple[List[Dict], List[int]]:
        ...
        严格模式: RmsNorm → 2个Linear → RmsNorm → 3个Linear → add_bias
        """
        ...
        while search_start_idx < len(all_kernels):
            ...
            # 检查是否符合2+3模式
            if len(attention_linear) == 2 and len(mlp_linear) == 3:
                layer_info = {
                ...
                print(f"  候选层不匹配: Attention Linear={len(attention_linear)}, MLP Linear={len(mlp_linear)} (期望2+3)")
                # 不符合模式，从第二个norm重新开始搜索（第二个norm作为新的第一个norm）
                search_start_idx = norm2_idx
                print(f"  不匹配，下次搜索从第二个norm索引 {search_start_idx} 重新开始")
        
        print(f"\n总共找到 {len(valid_layers)} 个有效的Transformer层")
    ...
    def calculate_projection_performance(self, layer_info: Dict, all_kernels: List[Dict], 
        ...
        """计算单个Transformer层中各projection的性能 - 使用精确token size"""
        ...
        # 分配kernels到projections
        all_linear_indices = layer_info['attention_linear'] + layer_info['mlp_linear']
    ...
    def analyze_linear_performance(self, all_kernels: List[Dict], model_config: Dict, 
        ...
        """分析Linear算子性能 - 使用精确token size"""
        
        print(f"\n=== Linear算子性能分析 (使用精确token size) ===")
        ...
        # 寻找Transformer层 - 注意：这里需要传入所有kernels，不是只有linear_kernels
        ...
        for layer_info in valid_layers:
            layer_perf = self.calculate_projection_performance(layer_info, all_kernels, model_config)
        ...
        return {
            ...
            'projection_analysis': "precise_transformer_layer_based_with_exact_token_size",
    ...
    def analyze_performance(self, processed_data: Dict[str, Any]) -> Dict[str, Any]:
        """主要的性能分析函数 - 专门分析Linear算子，使用精确token size"""
        
        print(f"\n=== OEA Stage 2: Linear算子专门分析 (使用精确token size) ===")
        ...
        result = {
            ...
            'analysis_method': 'precise_token_size_based'
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(description='OEA Stage 2 Linear算子分析器 (修复版本)')
