#!/usr/bin/env python3
"""
MEA Stage 1: Memory Anchor Detection and Iteration Segmentation (Fixed Version)
Modified approach for better first iteration detection:
1. First iteration: starts from first kernel event, ends at first DtoH memcpy
2. Remaining iterations: use original HtoD-DtoH pairing logic
"""
...
class MemoryAnchorDetectorFixed:
    """
    Stage 1: Memory Anchor Detection and Iteration Segmentation (Fixed Version)
    Modified to ensure first iteration starts from first kernel event
    """
    ...
    def find_first_kernel_event(self, trace_events: List[Dict[str, Any]]) -> float:
        """
        Find the timestamp of the first kernel event
        """
        for event in trace_events:
            if event.get("cat") == "kernel" and event.get("ph") == "X":
                return event.get("ts", 0)
        
        logger.warning("No kernel events found in trace")
        return 0
    
    def extract_candidate_segments_fixed(self, trace_events: List[Dict[str, Any]], 
                                        start_anchors: List[float], 
                                        end_anchors: List[float]) -> List[IterationCandidate]:
        """
        Extract candidate iteration segments with fixed first iteration logic:
        1. First iteration: from first kernel to first DtoH
        2. Remaining iterations: HtoD-DtoH pairing as before
        """
        candidates = []
        
        if not end_anchors:
            logger.warning("No DtoH anchors found - cannot detect iterations")
            return candidates
        
        # Find first kernel event timestamp
        first_kernel_ts = self.find_first_kernel_event(trace_events)
        if first_kernel_ts == 0:
            logger.warning("No kernel events found - falling back to original logic")
            return self.extract_candidate_segments_original(trace_events, start_anchors, end_anchors)
        
        logger.info(f"First kernel event at {first_kernel_ts:.1f}μs")
        ...
        # === First iteration: from first kernel to first DtoH ===
        first_dtoh = dtoh_sorted[0]
        first_iteration_duration = first_dtoh - first_kernel_ts
        
        logger.info(f"First iteration: {first_kernel_ts:.1f} -> {first_dtoh:.1f}μs (duration: {first_iteration_duration:.1f}μs)")
        
        if first_iteration_duration >= MIN_ITERATION_DURATION:
            # Extract events for first iteration
            first_iteration_events = []
            for event in trace_events:
                event_ts = event.get("ts", 0)
                if first_kernel_ts <= event_ts < first_dtoh:
                    first_iteration_events.append(event)
            
            if len(first_iteration_events) > 0:
                first_candidate = IterationCandidate(first_kernel_ts, first_dtoh, first_iteration_events)
                first_candidate.iteration_id = 1
                candidates.append(first_candidate)
                logger.info(f"Created first iteration: {len(first_iteration_events)} events, duration {first_iteration_duration:.1f}μs")
            else:
                logger.warning("First iteration has no events - skipping")
        else:
            logger.warning(f"First iteration duration {first_iteration_duration:.1f}μs < {MIN_ITERATION_DURATION}μs threshold - skipping")
        
        # === Remaining iterations: use original HtoD-DtoH pairing logic ===
        # Find HtoD events that start after first DtoH
        remaining_htod = [ts for ts in htod_sorted if ts >= first_dtoh]
        remaining_dtoh = dtoh_sorted[1:]  # Skip first DtoH as it's already used
        
        logger.info(f"Processing remaining iterations: {len(remaining_htod)} HtoD, {len(remaining_dtoh)} DtoH")
        
        # Track which DtoH events have been used
        used_dtoh = {first_dtoh}  # First DtoH is already used
        current_htod_idx = 0
        
        while current_htod_idx < len(remaining_htod):
            start_ts = remaining_htod[current_htod_idx]
            
            # Find the first unused DtoH after this HtoD that results in sufficient duration
            end_ts = None
            used_dtoh_ts = None
            
            for dtoh_ts in remaining_dtoh:
            ...
            if end_ts is None:
                logger.info(f"No more DtoH with sufficient duration available after HtoD at {start_ts:.1f}. Remaining {len(remaining_htod) - current_htod_idx} HtoD events are fragments.")
                break
            ...
            while next_htod_idx < len(remaining_htod) and remaining_htod[next_htod_idx] < end_ts:
                logger.debug(f"Skipping internal HtoD at {remaining_htod[next_htod_idx]:.1f} (within current iteration)")
                next_htod_idx += 1
            
            current_htod_idx = next_htod_idx
        
        logger.info(f"Extracted {len(candidates)} candidate segments (1 fixed first + {len(candidates)-1} regular)")
        
        # Log iteration statistics for debugging
        if candidates:
            durations = [c.duration for c in candidates]
            event_counts = [len(c.events) for c in candidates]
            logger.info(f"Duration stats: min={min(durations):.1f}μs, max={max(durations):.1f}μs, avg={sum(durations)/len(durations):.1f}μs")
            logger.info(f"Event count stats: min={min(event_counts)}, max={max(event_counts)}, avg={sum(event_counts)/len(event_counts):.1f}")
        
        return candidates
    
    def extract_candidate_segments_original(self, trace_events: List[Dict[str, Any]], 
                                          start_anchors: List[float], 
                                          end_anchors: List[float]) -> List[IterationCandidate]:
        """
        ...
        logger.info(f"Using original logic: {len(start_anchors)} HtoD and {len(end_anchors)} DtoH events")
    ...
    def detect_iterations(self, trace_file: str) -> List[IterationCandidate]:
        """
        Main method for Stage 1: Memory anchor detection and iteration segmentation (Fixed Version)
        """
        logger.info(f"Starting Stage 1 (Fixed): Memory anchor detection for {trace_file}")
        ...
        if len(end_anchors) == 0:
            logger.warning("No DtoH memory transfer events found. Cannot detect iterations.")
            return []
        
        # Extract candidate segments with fixed logic
        candidates = self.extract_candidate_segments_fixed(trace_events, start_anchors, end_anchors)
        
        logger.info(f"Stage 1 (Fixed) completed: {len(candidates)} candidate iterations detected")
        return candidates
    ...
    def save_results(self, candidates: List[IterationCandidate], output_file: str):
        ...
        results = {
            'stage': 1,
            'description': 'Memory anchor detection and iteration segmentation (Fixed Version - first iteration from first kernel)',
            'metadata': {
                'total_candidates': len(candidates),
                'stage_1_completed': True,
                'version': 'fixed_first_iteration'
            },
        ...
        logger.info(f"Stage 1 (Fixed) results saved to {output_file}")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python stage1_memory_anchor_detector_fixed.py <input_trace_file> [--output <output_file>]")
        print("Example: python stage1_memory_anchor_detector_fixed.py trace.json")
        print("         python stage1_memory_anchor_detector_fixed.py trace.json --output stage1_results.json")
        print("")
        print("Fixed Version: First iteration starts from first kernel event (not first HtoD)")
    ...
    try:
        ...
        if candidates:
            ...
            print(f"First iteration event count: {event_counts[0]} (should be larger than original)")
        print(f"Results saved to: {output_file}")
        print(f"\nReady for Stage 2: Operator fingerprint validation")
        
    except Exception as e:
        logger.error(f"Error during Stage 1 (Fixed) processing: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)
